        ...
        success: function (data) {
            ...
            document.querySelectorAll('.theater-place').forEach(list => {
                list.addEventListener('click', function () {
                    ...
                    theaterPlaceWrapper.forEach(list => {
                        list.classList.remove('theater-place-active');
                    });
            ...
            document.querySelectorAll('.reserve-time-want').forEach(list => {
                list.addEventListener('click', function () {
                    ...
                    theaterPlaceWrapper.forEach(list => {
                        list.classList.remove('reserve-time-active');
                    });
...
function setCity(data) {
    document.querySelector('.theater-place-wrapper').innerHTML = JSON.parse(
        data
    ).reduce((html = '', item, index = 0) => {
        html += getCityList(item);
...
function getCityList(item) {
    return `<button class="theater-place">${item.movieTable}</button>`;
}

function setInfo(data) {
    document.querySelector('.reserve-time').innerHTML = JSON.parse(
        data
    ).reduce((html = '', item, index = 0) => {
        html += getTimeList(item);

        return html;
    }, ' ');
}

function getTimeList(item) {
    console.log(item);
    return `<div class="reserve-where">${item.theater_no}</div>
    <div class="reserve-time-wrapper">
        <button class="reserve-time-button">
            <span class="reserve-time-want">${item.theater_time}</span>
            <span class="reserve-time-remain">${item.theater_seat}</span>
        </button>
    </div> `;
}
// 데이터 가져오기
